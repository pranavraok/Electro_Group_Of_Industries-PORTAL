...
5. Save the script. Do not run a setup function; the verified Spreadsheet ID is already configured. The `calibration_reports` and `offer_letters` tabs are created automatically on first use.
...
Calibration report and offer-letter saves are designed for this internal operations interface and do not use the wire-database admin password. If the site will be public, restrict the Web App deployment to your organisation before using shared record storage.
